...
namespace DTcms.Web.UI.Page
{
    public partial class oauth_login : Web.UI.BasePage
    {
        ...
        void UserPage_Init(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(turl) || turl == System.Web.HttpContext.Current.Request.Url.ToString().ToLower())
            {
            ...
            {
                System.Web.HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Utils.UrlEncode("登录失败，用户授权已过期，请重新登录！")));
                return;
            }
            Model.user_oauth oauthModel = new BLL.user_oauth().GetModel(Session["oauth_name"].ToString(), Session["oauth_openid"].ToString());
            if (oauthModel != null)
            {
                //检查用户是否存在
                ...
                if (model == null)
                {
                    System.Web.HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Utils.UrlEncode("登录失败，授权用户不存在或已被删除！")));
                    return;
                }
                ...
                //更新最新的Access Token
                oauthModel.oauth_access_token = Session["oauth_access_token"].ToString();
