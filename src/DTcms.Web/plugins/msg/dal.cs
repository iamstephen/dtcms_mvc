...
namespace DTcms.Web.plugins.msg
{
    public class dal
    {
        private string databaseprefix; //数据库表名前缀
        public dal(string _databaseprefix)
        {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(feedback model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into " + databaseprefix + "feedback(");
            strSql.Append("site_id,title,content,user_name,user_tel,user_qq,user_email,add_time,is_lock)");
            strSql.Append(" values (");
            strSql.Append("@site_id,@title,@content,@user_name,@user_tel,@user_qq,@user_email,@add_time,@is_lock)");
            strSql.Append(";select @@IDENTITY");
            SqlParameter[] parameters = {
                    new SqlParameter("@site_id", SqlDbType.NVarChar,100),
        ...
        public feedback GetModel(int id)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["site_id"] != null && ds.Tables[0].Rows[0]["site_id"].ToString() != "")
                {
                    model.site_path = ds.Tables[0].Rows[0]["site_id"].ToString();
                }
        ...
        public DataSet GetList(int Top, string strWhere)
        {
            ...
            strSql.Append(" id,site_id,title,content,user_name,user_tel,user_qq,user_email,add_time,reply_content,reply_time,is_lock ");
            strSql.Append(" FROM " + databaseprefix + "feedback ");
            if (strWhere.Trim() != "")
            {
